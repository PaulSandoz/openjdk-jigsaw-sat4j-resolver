...
class SatTrace {
    ...
    static {
        PrivilegedAction<String> pa = new PrivilegedAction<String>() {
            @Override
            public String run() {
                return System.getenv("JIGSAW_TRACE_SAT");
            }
        };
    ...
    // SatTrace.trace method may be called while the classes for tracing
    // e.g. java.util.Formatter) are being loaded.  Cache the traces
    // to avoid infinite loop until the very first call completes.
    static class Cache {

        static List<String> formats = new ArrayList<>();

        static List<Object[]> traceArgs = new ArrayList<>();
